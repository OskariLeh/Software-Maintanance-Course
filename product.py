...
class productClass:
    def __init__(self,root):
        ...
        lbl_barcode=Label(product_Frame,text="Barcode",font=("goudy old style",18),bg="white").place(x=30,y=310)
        ...
        txt_qty=Entry(product_Frame,textvariable=self.var_barcode,font=("goudy old style",15),bg="lightyellow").place(x=150,y=310,width=200)
        ...
        btn_clear=Button(product_Frame,text="Clear",command=self.clear,font=("goudy old style",15),bg="#607d8b",fg="white",cursor="hand2").place(x=340,y=430,width=100,height=40)
        ...
        #------------ options ----------------
        cmb_search=ttk.Combobox(SearchFrame,textvariable=self.var_searchby,values=("Select","Category","Supplier","Name", "Barcode"),state='readonly',justify=CENTER,font=("goudy old style",15))
        ...
        self.ProductTable=ttk.Treeview(product_frame,columns=("pid","Category","Supplier","name","price","qty","status", "barcode"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        ...
        try:
            if self.var_cat.get()=="Select" or self.var_cat.get()=="Empty" or self.var_sup=="Select" or self.var_sup=="Empty":
                ...
                if row!=None:
                    ...
                    cur.execute("insert into product(Category,Supplier,name,price,qty,status,barcode) values(?,?,?,?,?,?,?)",(
                        self.var_cat.get(),
                        self.var_sup.get(),
                        self.var_name.get(),
                        self.var_price.get(),
        ...
        try:
            if self.var_pid.get()=="":
                ...
                if row==None:
                    ...
                    cur.execute("update product set Category=?,Supplier=?,name=?,price=?,qty=?,status=?, barcode=? where pid=?",(
                        self.var_cat.get(),
                        self.var_sup.get(),
                        self.var_name.get(),
                        self.var_price.get(),
