...
'''
    ...
    def print_bill(self):
        if self.chk_print==1:
            ...
            ## TODO Removed depradacted function mktemp -> mkstemp
            new_file=tempfile.mkstemp(suffix='.txt')
            open(new_file[1],'w').write(self.txt_bill_area.get('1.0',END))
            os.startfile(new_file[1],'print')
        else:
